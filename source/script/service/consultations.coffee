...
angular.module("app")
  .service "ConsultationGenerator", ["$http", class
    ...
    generate: () ->
      consultation = {
        id: @seq++
        name: randomize(kConsultationNames)
        max: 3
        actual: 0
        desc: randomize(kDescriptions)
        date: randomDate(new Date(), new Date(2016, 4, 31))
        place: randomize(kPlaces)
        consultant: randomize(kConsultantNames)
        registered: false
      }
